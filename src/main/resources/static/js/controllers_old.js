...
angular.module('springChat.controllers', ['toaster', 'ngFileUpload'])
	.controller('ChatController', ['$scope', '$location', '$interval', 'toaster', 'ChatSocket', '$http', 'Upload', function($scope, $location, $interval, toaster, chatSocket, $http, Upload) {
		...
		$scope.sendMessage = function() {
			...
			$scope.messages.push({date: new Date() ,message: $scope.newMessage, username: $scope.username, to: $scope.sendTo});
            $("html, body").animate({ scrollTop: $(document).height() }, 1000);

			chatSocket.send(destination, {}, JSON.stringify({date: new Date().toUTCString() ,message: $scope.newMessage, to: $scope.sendTo}));
			$scope.newMessage = '';
		};
		...
		$scope.startTyping = function() {
	        ...
	        typing = $interval(function() { $scope.stopTyping();}, 500);

	        chatSocket.send("/app/chat.typing", {}, JSON.stringify({from: $scope.username, to: $scope.sendTo, typing: true}));
		};
		...
		$scope.stopTyping = function() {
			if (angular.isDefined(typing)) {
		        ...
		        chatSocket.send("/app/chat.typing", {}, JSON.stringify({from: $scope.username, to: $scope.sendTo, typing: false}));
			}
		};
